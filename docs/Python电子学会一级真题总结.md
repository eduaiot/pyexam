...
`turtle.begin\_fill() turtle.end\_fill()`：标记图形填充颜色的起点和结束，必须成对使用。与fillcolor和color做配合。
**注意**：color只写了一个参数时，如果用到了这两个函数，则填充该颜色。如果同时写color("red")，fillcolor("yellow")则可以认为是color("red"，"yellow")
![](./attachments/根据试卷总结知识点_006.png)
* 示例
...
* ![](./attachments/根据试卷总结知识点_007.png)错
#### 11.6. turtle圆
`circle与dot`区别
...
```
* ![](./attachments/根据试卷总结知识点_008.png)
### 13-数学问题与逻辑推理
* ![](./attachments/根据试卷总结知识点_009.png)
* ![](./attachments/根据试卷总结知识点_010.png)![](./attachments/根据试卷总结知识点_011.png)